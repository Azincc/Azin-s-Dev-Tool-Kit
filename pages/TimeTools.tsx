...
export const WorldClockTools: React.FC = () => {
    ...
    const zones = [
        ...
        { labelKey: 'tool.worldclock.city.los_angeles', zone: 'America/Los_Angeles' },
    ...
    const getOffsetStatus = (offsetMs: number) => {
        ...
        if (absOffset < 300000) return { color: 'text-yellow-500', message: t('tool.worldclock.warn_totp') };
        if (absOffset < 3600000) return { color: 'text-orange-500', message: t('tool.worldclock.warn_jwt') };
                        ...
                        {timeOffset !== null && (
                            <div className={`text-sm ${getOffsetStatus(timeOffset).color}`}>
                                {(() => {
                                    ...
                                    const direction = timeOffset < 0 ? t('tool.worldclock.slow') : t('tool.worldclock.fast');
                                    ...
                                    if (language === 'zh') {
                                        ...
                                        result = `${t('tool.worldclock.diff_prefix')}${timeStr} ${direction}${t('tool.worldclock.diff_suffix')}`;
                                    }
