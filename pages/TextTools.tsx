  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2"><Card className="h-full flex flex-col"><CardHeader title={t('tool.editor.editor')} /><TextArea value={text} onChange={e => setText(e.target.value)} className="flex-1 min-h-[400px] border-0 p-4 resize-none" placeholder={t('tool.editor.placeholder')} /></Card></div>
            <div className="space-y-6">
    ...
    <div className="space-y-6">
        ...
        <Card>
            <CardContent className="space-y-4 p-6">
                <div className="flex gap-2"><Input value={regexPattern} onChange={e => setRegexPattern(e.target.value)} placeholder={t('tool.regex.pattern') + " (e.g. \\d+)"} className="flex-1 font-mono text-emerald-600 dark:text-emerald-400" /><Input value={regexFlags} onChange={e => setRegexFlags(e.target.value)} placeholder={t('tool.regex.flags')} className="w-24 font-mono" /></div>
...
export const DiffTools: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 h-[400px]"><div className="flex flex-col gap-2 h-full"><Label>{t('tool.diff.original')}</Label><TextArea value={diffLeft} onChange={e => setDiffLeft(e.target.value)} className="flex-1 resize-none font-mono text-xs whitespace-pre" /></div><div className="flex flex-col gap-2 h-full"><Label>{t('tool.diff.changed')}</Label><TextArea value={diffRight} onChange={e => setDiffRight(e.target.value)} className="flex-1 resize-none font-mono text-xs whitespace-pre" /></div></div>
        <Card><CardHeader title={t('tool.diff.result')} /><div className="p-0 bg-slate-100 dark:bg-slate-950 overflow-auto max-h-[400px]"><table className="w-full font-mono text-xs border-collapse"><tbody>{diffResult.map((row) => (<tr key={row.idx} className={row.type === 'diff' ? 'bg-yellow-100 dark:bg-yellow-900/20' : ''}><td className="w-12 p-1 text-slate-400 dark:text-slate-600 text-right select-none border-r border-slate-200 dark:border-slate-800">{row.idx}</td><td className={`p-1 w-1/2 border-r border-slate-200 dark:border-slate-800 ${row.type === 'diff' ? 'bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-200' : 'text-slate-600 dark:text-slate-400'}`}>{row.l}</td><td className={`p-1 w-1/2 ${row.type === 'diff' ? 'bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-200' : 'text-slate-600 dark:text-slate-400'}`}>{row.r}</td></tr>))}</tbody></table></div></Card>
    </div>
  );
};
