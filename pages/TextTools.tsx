    ...
    <div className="space-y-6">
        ...
        <Card>
            <CardContent className="space-y-4 p-6">
                <div className="flex gap-2 items-start"><TextArea value={regexPattern} onChange={e => setRegexPattern(e.target.value)} placeholder={t('tool.regex.pattern') + " (e.g. \\d+)"} className="flex-1 min-w-0 font-mono text-emerald-600 dark:text-emerald-400 bg-slate-50 dark:bg-slate-900 text-slate-900 !min-h-[42px] !h-[42px]" /><Input value={regexFlags} onChange={e => setRegexFlags(e.target.value)} placeholder={t('tool.regex.flags')} className="w-16 font-mono bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white" /></div>
