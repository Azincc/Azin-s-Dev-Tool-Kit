...
export const CsvTools: React.FC = () => {
  ...
  const exportSql = () => {
    ...
    const table =
      fileName.split(".")[0].replace(/[^a-zA-Z0-9_]/g, "_") || "table_name";
    const cols = headers
      ...
      <div className="flex justify-between items-start">
        ...
        {data.length > 0 && (
          <div className="flex gap-2">
            ...
            <Button onClick={exportSql} variant="secondary" size="sm">
              {t("tool.csv.toSql")}
            </Button>
              ...
              <span className="text-sm text-slate-500">
                {
                  t(
                    "tool.csv.rows"
                  ) /* reusing label for 'Sheet' if lazy, but better add specific label */
                }
                :
              </span>
              <Select
          ...
          {data.length > 0 && (
            <div className="flex items-center gap-4">
              <div className="text-xs text-slate-500">
                {data.length} {t("tool.csv.rows")} × {headers.length}{" "}
                {t("tool.csv.cols")}
              </div>
              <input
                type="text"
            ...
            <table className="w-full text-sm text-left border-collapse">
              <thead className="text-xs text-slate-700 uppercase bg-slate-100 dark:bg-slate-900 dark:text-slate-400 sticky top-0 z-10">
                <tr>
                  {headers.map((header) => (
                    <th
                      key={header}
                      className="px-6 py-3 border-b border-slate-200 dark:border-slate-700 font-medium whitespace-nowrap"
                    >
                      {header}
                    </th>
                  ))}
              ...
              <tbody>
                {filteredData.slice(0, 100).map((row, i) => (
                  <tr
                    key={i}
                    className="bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-900"
                  >
                    {headers.map((header) => (
                      <td
                        key={`${i}-${header}`}
                        className="px-6 py-4 whitespace-nowrap text-slate-900 dark:text-slate-300 font-mono text-xs"
                      >
