  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card><CardHeader title={t('tool.image.qr_title')} /><CardContent className="space-y-4"><Input value={qrText} onChange={e => setQrText(e.target.value)} placeholder={t('tool.image.qr_placeholder')} className="bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white" /><div className="flex justify-center bg-white p-4 rounded-lg w-fit mx-auto border border-slate-200 dark:border-none"><img src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(qrText)}`} alt="QR" className="w-32 h-32" /></div></CardContent></Card>
        </div>
    </div>
  );
};
