...
export const ImageTools: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
        <div><h2 className="text-2xl font-bold text-text-primary">{t('tool.image.title')}</h2><p className="text-text-secondary">{t('tool.image.desc')}</p></div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card><CardHeader title={t('tool.image.base64_title')} /><CardContent className="space-y-4"><input type="file" onChange={handleImageUpload} className="block w-full text-sm text-text-secondary file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:bg-background-secondary file:text-text-primary" />{imgBase64 && (<div className="space-y-2"><div className="h-32 w-full bg-background-secondary rounded flex items-center justify-center overflow-hidden border border-border"><img src={imgBase64} alt="Preview" className="max-h-full max-w-full" /></div><div className="relative"><Input readOnly value={imgBase64.substring(0, 50) + "..."} /><div className="absolute top-1 right-1"><CopyButton text={imgBase64} /></div></div></div>)}</CardContent></Card>
            <Card><CardHeader title={t('tool.image.qr_title')} /><CardContent className="space-y-4"><Input value={qrText} onChange={e => setQrText(e.target.value)} placeholder={t('tool.image.qr_placeholder')} className="w-full" /><div className="flex justify-center bg-background-surface p-4 rounded-lg w-fit mx-auto border border-border dark:border-none"><img src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(qrText)}`} alt="QR" className="w-32 h-32" /></div></CardContent></Card>
        </div>
    </div>
  );
};
