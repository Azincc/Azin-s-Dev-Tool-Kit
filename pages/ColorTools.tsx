  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card className="col-span-1"><CardHeader title={t('tool.color.picker')} /><CardContent className="flex flex-col items-center py-8 gap-4"><input type="color" value={hex} onChange={e => setHex(e.target.value)} className="w-24 h-24 rounded-full cursor-pointer overflow-hidden border-0" /><Input value={hex} onChange={e => setHex(e.target.value)} className="text-center font-mono" /></CardContent></Card>
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card><CardHeader title={t('tool.image.qr_title')} /><CardContent className="space-y-4"><Input value={qrText} onChange={e => setQrText(e.target.value)} placeholder={t('tool.image.qr_placeholder')} /><div className="flex justify-center bg-white p-4 rounded-lg w-fit mx-auto border border-slate-200 dark:border-none"><img src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(qrText)}`} alt="QR" className="w-32 h-32" /></div></CardContent></Card>
        </div>
    </div>
  );
};
