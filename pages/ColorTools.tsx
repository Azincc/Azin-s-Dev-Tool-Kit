  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card className="col-span-1"><CardHeader title="Picker" /><CardContent className="flex flex-col items-center py-8 gap-4"><input type="color" value={hex} onChange={e => setHex(e.target.value)} className="w-24 h-24 rounded-full cursor-pointer overflow-hidden border-0" /><Input value={hex} onChange={e => setHex(e.target.value)} className="text-center font-mono" /></CardContent></Card>
            <Card className="col-span-2"><CardHeader title="Shades" /><div className="flex h-32 w-full px-6 pb-6">{[100, 200, 300, 400, 500, 600, 700, 800, 900].map((w, i) => (<div key={w} className="flex-1 flex items-end p-2 text-xs" style={{ backgroundColor: hex, filter: `brightness(${1.5 - (i * 0.1)})` }}><span className={i > 4 ? 'text-white' : 'text-black'}>{w}</span></div>))}</div></Card>
        </div>
    </div>
  );
};
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card><CardHeader title="Image to Base64" /><CardContent className="space-y-4"><input type="file" onChange={handleImageUpload} className="block w-full text-sm text-slate-500 dark:text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:bg-slate-200 dark:file:bg-slate-700 file:text-slate-700 dark:file:text-white" />{imgBase64 && (<div className="space-y-2"><div className="h-32 w-full bg-slate-100 dark:bg-slate-900 rounded flex items-center justify-center overflow-hidden border border-slate-200 dark:border-slate-700"><img src={imgBase64} alt="Preview" className="max-h-full max-w-full" /></div><div className="relative"><Input readOnly value={imgBase64.substring(0, 50) + "..."} /><div className="absolute top-1 right-1"><CopyButton text={imgBase64} /></div></div></div>)}</CardContent></Card>
            <Card><CardHeader title="QR Code Generator" /><CardContent className="space-y-4"><Input value={qrText} onChange={e => setQrText(e.target.value)} placeholder="Enter URL or Text" /><div className="flex justify-center bg-white p-4 rounded-lg w-fit mx-auto border border-slate-200 dark:border-none"><img src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(qrText)}`} alt="QR" className="w-32 h-32" /></div></CardContent></Card>
        </div>
    </div>
  );
};
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <Card><CardHeader title="Controls" /><CardContent className="space-y-6"><div className="space-y-4"><Label>Border Radius: {borderRadius}px</Label><Slider value={borderRadius} min={0} max={50} onChange={(e) => setBorderRadius(Number(e.target.value))} /></div><div className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-700"><Label>Box Shadow</Label><div className="grid grid-cols-2 gap-4"><div><Label>X</Label><Slider min={-50} max={50} value={boxShadow.x} onChange={e => setBoxShadow({...boxShadow, x: Number(e.target.value)})} /></div><div><Label>Y</Label><Slider min={-50} max={50} value={boxShadow.y} onChange={e => setBoxShadow({...boxShadow, y: Number(e.target.value)})} /></div><div><Label>Blur</Label><Slider min={0} max={100} value={boxShadow.blur} onChange={e => setBoxShadow({...boxShadow, blur: Number(e.target.value)})} /></div><div><Label>Spread</Label><Slider min={-20} max={50} value={boxShadow.spread} onChange={e => setBoxShadow({...boxShadow, spread: Number(e.target.value)})} /></div></div></div></CardContent></Card>
            <div className="space-y-6"><div className="h-48 bg-slate-100 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 flex items-center justify-center"><div className="w-32 h-32 bg-blue-500 transition-all duration-200" style={{ borderRadius: `${borderRadius}px`, boxShadow: shadowStyle }}></div></div><div className="relative"><Label>CSS Output</Label><TextArea readOnly value={`border-radius: ${borderRadius}px;\nbox-shadow: ${shadowStyle};`} className="bg-slate-50 dark:bg-slate-950 text-purple-600 dark:text-purple-400 h-24" /><div className="absolute top-8 right-2"><CopyButton text={`border-radius: ${borderRadius}px;\nbox-shadow: ${shadowStyle};`} /></div></div></div>
        </div>
    </div>
  );
};