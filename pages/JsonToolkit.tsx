  ...
  return (
    <div className="space-y-6 h-[calc(100vh-4rem)] flex flex-col">
      ...
      <div className="flex flex-wrap gap-2 bg-slate-100 dark:bg-slate-800 p-2 rounded-lg border border-slate-200 dark:border-slate-700">
        {[{id: 'format', l: 'Prettify'}, {id: 'minify', l: 'Minify'}, {id: 'toTS', l: 'To TS'}, {id: 'toGo', l: 'To Go'},
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1 min-h-0">
        <Card className="flex flex-col h-full">
          <CardHeader title="Input JSON" action={input && <button onClick={() => { setInput(''); setOutput(''); }} className="text-slate-400 hover:text-red-400"><TrashIcon className="w-4 h-4" /></button>} />
        ...
        <Card className="flex flex-col h-full border-blue-900/30">
          <CardHeader title={error ? "Error" : "Output"} action={<CopyButton text={output} />} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1 min-h-0">
        <Card className="flex flex-col h-full">
          <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 flex justify-between items-center">
              <h3 className="font-semibold text-slate-800 dark:text-white">Input Code</h3>
          ...
          <TextArea value={codeInput} onChange={(e) => setCodeInput(e.target.value)} className="flex-1 w-full border-0 bg-slate-50 dark:bg-slate-900 p-4 resize-none font-mono text-sm" placeholder={`Paste ${codeLang.toUpperCase()} here...`} spellCheck={false} />
        </Card>
        <Card className="flex flex-col h-full border-blue-900/30">
            <CardHeader title="Formatted Output" action={<CopyButton text={codeOutput} />} />
          ...
          <CardContent className="space-y-6 p-6">
               <div className="flex flex-col gap-4">
                   <div className="flex gap-4 items-center">
                       <Label className="mb-0 w-24">Mode</Label>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                       <div className="space-y-2">
                           <Label>Input</Label>
                           <TextArea value={encodeInput} onChange={(e) => setEncodeInput(e.target.value)} className="h-64" placeholder="Type here..." />
                       </div>
                       <div className="space-y-2">
                           <Label>Result</Label>
