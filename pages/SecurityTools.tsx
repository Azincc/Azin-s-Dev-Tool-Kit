...
export const HashTools: React.FC = () => {
  ...
  const handleFileHash = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setHashes({ sha256: toHex(sha256), sha1: "Skipped for file", sha512: "Skipped for file" });
  };
          ...
          <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                      <Label>Text</Label>
                      <Input value={hashInput.startsWith("File:") ? "" : hashInput} onChange={(e) => calculateHashes(e.target.value)} placeholder="Type to hash..." />
                  </div>
                  <div className="space-y-2">
                      <Label>Or File</Label>
              ...
              <div className="space-y-4">
                  {['sha1', 'sha256', 'sha512'].map(algo => (
                      <div key={algo}>
                          ...
                          <div className="bg-slate-100 dark:bg-slate-950 p-3 rounded border border-slate-200 dark:border-slate-700 font-mono text-xs text-slate-600 dark:text-slate-300 break-all">{hashes[algo] || "Waiting for input..."}</div>
                      </div>
                  ))}
              </div>
          </CardContent>
...
export const EncryptTools: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
        ...
        <Card>
            <CardHeader title="XOR Cipher Demo" />
            <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2"><Label>Message</Label><TextArea value={aesInput} onChange={e => setAesInput(e.target.value)} placeholder="Secret Message" /></div>
                    <div className="space-y-2"><Label>Secret Key</Label><Input value={aesKey} onChange={e => setAesKey(e.target.value)} /><Button onClick={simpleEncrypt} className="w-full mt-2">Encrypt / Decrypt</Button></div>
                </div>
                <div className="space-y-2"><Label>Result</Label><div className="relative"><TextArea readOnly value={aesOutput} className="bg-slate-100 dark:bg-slate-950 text-emerald-600 dark:text-emerald-400" /><div className="absolute top-2 right-2"><CopyButton text={aesOutput} /></div></div></div>
            </CardContent>
        </Card>
    </div>
  );
    ...
    <div className="space-y-6">
        ...
        <Card>
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6">
                <div className="space-y-2"><Label>Encoded Token</Label><TextArea value={jwtToken} onChange={e => decodeJwt(e.target.value)} className="h-64 text-xs" placeholder="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..." /></div>
                <div className="space-y-4">
                    <div className="space-y-1"><Label className="text-blue-600 dark:text-blue-400">Header</Label><pre className="bg-slate-100 dark:bg-slate-950 p-3 rounded border border-slate-200 dark:border-slate-700 text-xs text-slate-800 dark:text-slate-300 h-24 overflow-auto">{jwtDecoded?.header}</pre></div>
                    <div className="space-y-1"><Label className="text-purple-600 dark:text-purple-400">Payload</Label><pre className="bg-slate-100 dark:bg-slate-950 p-3 rounded border border-slate-200 dark:border-slate-700 text-xs text-slate-800 dark:text-slate-300 h-32 overflow-auto">{jwtDecoded?.payload}</pre></div>
                </div>
            </CardContent>
        </Card>
    </div>
    ...
    <div className="space-y-6">
        ...
        <Card>
            <CardContent className="space-y-4 pt-6">
                <div className="flex gap-4 items-end"><div className="w-32"><Label>Length</Label><Input type="number" value={passLength} onChange={e => setPassLength(parseInt(e.target.value))} min={4} max={128} /></div><Button onClick={generatePasswords}>Generate Batch</Button></div>
                <div className="grid gap-2">
                    ...
                    {passwords.length === 0 && <div className="text-slate-500 text-center py-4">Click generate</div>}
                </div>
            </CardContent>
        </Card>
    </div>
